...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr class="odd">
		<td>所属部门：</td>
		<td>
			<!--当前用户的deptId与deptList中的deptId如果一致 则选中  -->
	...
	<tr class="odd">
		...
		<td>直属领导：</td>
		<td>
			<select name="userInfo.managerId.userInfoId" style="width:121px">
				<option value="">---请选择/无上级---</option>
				<c:forEach items="${managerList}" var="m">
					<c:if test="${m.userInfoId != user.userId}">
						<option value="${m.userInfoId}" 
						<c:if test="${m.userInfoId == user.userInfo.managerId.userInfoId}">selected="selected"</c:if>>${m.name}</option>
					</c:if>
				</c:forEach>
			</select>
		</td>
	...
	<tr class="odd">
		<td>入职时间：</td>
		<td><input name="userInfo.joinDate" type="text" onclick="WdatePicker({el:this,isShowOthers:true,dateFmt:'yyyy-MM-dd'});" 
		value="<fmt:formatDate value="${user.userInfo.joinDate}" pattern="yyyy-MM-dd"/>"/></td>
		<td>薪水：</td>
		<td><input type="text" name="userInfo.salary" value="${user.userInfo.salary}"/></td>
	</tr>
	<tr class="odd">
		<td>生日：</td>
		<td><input name="userInfo.birthday" type="text" onclick="WdatePicker({el:this,isShowOthers:true,dateFmt:'yyyy-MM-dd'});"
		 value="<fmt:formatDate value="${user.userInfo.birthday}" pattern="yyyy-MM-dd"/>"/></td>
		<td>性别：</td>
		<td>
